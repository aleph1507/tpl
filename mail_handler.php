...
<?php 
	//if(isset($_POST['submit'])){
		...
		$secret_key = '6LdiexAUAAAAAF2WHjEq3nlpHMDLPp1BRTzZin5Q';
		$response = $_POST['g-recaptcha-response'];
		$user_ip = $_SERVER['REMOTE_ADDR'];
		// if($response == "")
		// 	echo 'empty string<br>';
		// var_dump($response);
		// die('<br>grecaptcha echoed');

		// $captcha_verify = open_https_url("https://www.google.com/recaptcha/api/siteverify?secret=$secret_key&response=$client_captcha_response&remoteip=$user_ip");
		...
		// var_dump($verify);
		// die('<br>verify echoed');

		$verify_decoded = json_decode($verify, TRUE);
		// var_dump($verify_decoded);
		// echo '<br>verify_decoded echoed';


		 // if(!$verify)
		 //  	die('Please verify that you are not a robot.');

		 // $captcha_verify_decoded = json_decode($verify);
   ...
   //   	 }
		...
		if($verify_decoded['success']==FALSE){
		 	echo 'verify decoded false';
		 	die('Spam.');
		}
